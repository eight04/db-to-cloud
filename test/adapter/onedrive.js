...
const SCOPE = encodeURIComponent("Files.ReadWrite.AppFolder");
...
async function getOneDriveAccessToken()  {
  ...
  console.log(`https://login.microsoftonline.com/common/oauth2/v2.0/authorize?client_id=${process.env.AZURE_APP_ID}&scope=${SCOPE}&response_type=code&redirect_uri=https://login.microsoftonline.com/common/oauth2/nativeclient`);
  ...
  const res = await fetch("https://login.microsoftonline.com/common/oauth2/v2.0/token", {
    ...
    body: `client_id=${process.env.AZURE_APP_ID}&redirect_uri=https://login.microsoftonline.com/common/oauth2/nativeclient&code=${code}&grant_type=authorization_code&scope=${SCOPE}`
  });
