...
function createRequest({fetch, cooldown = 0, getAccessToken, username, password}) {
  ...
  async function doRequest({
    ...
    const headers = {};
    if (getAccessToken) {
      // A plain string means Bearer, for backward compat with every
      // existing caller. {scheme, param} lets a caller (e.g. the github
      // drive, for a "token <PAT>" header) ask for a different scheme.
      const token = await getAccessToken();
      headers["Authorization"] = typeof token === "string"
        ? `Bearer ${token}`
        : `${token.scheme || "Bearer"} ${token.param}`;
    }
    if (basicAuth) {
